use crate::{
    ...
    repository::{self, transaction::Filters},
    types::Context,
...
async fn get_transaction_by_id(
    ...
    match repository::transaction::find_by_id(&ctx.db_conn.pool, id).await {
...
async fn get_transactions(
    ...
    match repository::transaction::find_many(&ctx.db_conn.pool, pagination, filters).await {
        Ok(txs) => (StatusCode::OK, Json(json!(txs))),
        Err(_) => (
            StatusCode::INTERNAL_SERVER_ERROR,
